...
thisdir = pathlib.Path(_file_).parent.absolute()
...
async def convert_audio(file: UploadFile):
    ...
    try:
        ...
        if len(audio_np) < 16000:
            print(f"⚠ Audio too short ({len(audio_np)} samples), padding to 16000 samples...")
...
async def play_next_question():
    ...
    print(f"🎙 Playing Question {session_state['current_question_index'] + 1}: {question_text}")
